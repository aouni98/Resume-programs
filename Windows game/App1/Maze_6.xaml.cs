...
namespace App1
{
    public sealed partial class Maze_6 : Page
    {
        //Character Character;
        Random random;
        Sword weapon;
        ...
        BreakableWall Door = new BreakableWall();
        ...
        private async void CanvasAnimatedControl_Update(ICanvasAnimatedControl sender, CanvasAnimatedUpdateEventArgs args)
        {
            ...
            await Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
            {
                ...
                {
                    if (Character.X_Location + 100 >= Door.X_Location)
                    {
                        Broken.IfBroken = true;
                        walls.Remove(Door);
                    }
                }
                ...
                if (Character.X_Location > 960)
                {
                    ...
                    Character.Y_Location = 440;

                    this.Frame.Navigate(typeof(Maze_7));
                }
                if (Character.X_Location < 0)
                {
                    Character.X_Location = 910;
                    this.Frame.Navigate(typeof(Maze_1));
                }
            });
        }
        ...
        void CanvasControl_Draw(ICanvasAnimatedControl sender, CanvasAnimatedDrawEventArgs args)
        {
            ...
            foreach (Curtis_sRect x in walls)
            {
                ...
                else if (!Broken.IfBroken)
                    args.DrawingSession.DrawRectangle(Door.X_Location, Door.Y_Location, 20, 1000, Colors.RosyBrown, 20);
